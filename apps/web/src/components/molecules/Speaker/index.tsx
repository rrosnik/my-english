...
const Speaker = (props: SpeakerProps) => {
  ...
  const utter = () => {
    ...
    if (isAbleToSpeech()) {
      ...
      msg.lang = lang;
      msg.rate = 0.8;
      msg.onend = () => setIsSpeeching(false);
      msg.onerror = () => setIsSpeeching(false);
  ...
  return (
    <div className="speaker" onClick={changeState}>
      {text && <>{!isSpeeching ? <Icon icon="material-symbols:play-circle" width="24" /> : <Icon icon="material-symbols:stop-rounded" width="24" />}</>}
    </div>
  );
};
