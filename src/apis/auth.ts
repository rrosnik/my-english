...
export const createUser = (displayName: string, email: string, password: string) => {
  // create user
  return firebase.utils.auth.createUserWithEmailAndPassword(email, password).then<UserType>((userCredential) => {
    return firebase.auths.updateProfile(userCredential.user, { displayName }).then<UserType>(() => {
