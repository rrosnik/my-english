...
export const getCollections = () => {
  ...
  return firebase.fsDatabase.getDoc(colRef).then<Array<string>>((value) => {
...
export const getItems = (colId: string): Promise<Array<EnglishCard>> => {
  ...
  return firebase.fsDatabase.getDocs(colRef).then<Array<EnglishCard>>((docs) => {
    const items: Array<EnglishCard> = [];
    docs.forEach((doc) => {
      if (doc.id !== firebase.auth.currentUser?.uid) items.push({ id: doc.id, ...doc.data() } as EnglishCard);
    });
